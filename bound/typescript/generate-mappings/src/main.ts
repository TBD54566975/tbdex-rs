...
type WasmType = {
  ...
  constructorParams: Array<string>; // ordered as-is in the ts.Node & same names as properties.name
};
...
//   ts.forEachChild(
//     sourceFile,
//     (node) => ts.isClassDeclaration(node) && console.log(node)
//   )
// );

// ---

// function getTypeAsString(type: ts.TypeNode | undefined): string {
//   if (!type) {
//     return "any"; // If the type is not explicitly specified
//   }
//   return type.getText();
// }

// function logClassDetails(node: ts.Node) {
//   if (ts.isClassDeclaration(node) && node.name) {
...
//         console.log(`  Constructor: ${member.parameters.length} parameters`);
//         member.parameters.forEach((param: any) => {
//           const paramName = (param.name as ts.Identifier).text;
//           const paramType = getTypeAsString(param.type);
//           console.log(`    Parameter: ${paramName}, Type: ${paramType}`);
//         });
//       } else if (ts.isPropertyDeclaration(member) && member.name) {
//         const propertyName = (member.name as ts.Identifier).text;
//         const propertyType = getTypeAsString(member.type);
//         console.log(`  Property: ${propertyName}, Type: ${propertyType}`);
//       }
//     });
//   }

//   ts.forEachChild(node, logClassDetails); // Recursively visit children
// }

// logClassDetails(sourceFile);
