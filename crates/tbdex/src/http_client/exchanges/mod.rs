...
use http_body::{HttpBody, HttpBodyMessage};
use reqwest::Method;
...
pub fn create_exchange(rfq: &Rfq, reply_to: Option<String>) -> Result<()> {
    ...
    send_request::<HttpBody, ()>(
        ...
        Method::POST,
        Some(&HttpBody {
            message: Some(HttpBodyMessage::Rfq(rfq.clone())),
            reply_to,
        }),
        None,
    )?;
...
pub fn submit_order(order: &Order) -> Result<()> {
    ...
    send_request::<HttpBody, ()>(
        ...
        Method::PUT,
        Some(&HttpBody {
            message: Some(HttpBodyMessage::Order(order.clone())),
            ..Default::default()
        }),
        None,
    )?;
...
pub fn submit_cancel(cancel: &Cancel) -> Result<()> {
    ...
    send_request::<HttpBody, ()>(
        ...
        Method::PUT,
        Some(&HttpBody {
            message: Some(HttpBodyMessage::Cancel(cancel.clone())),
            ..Default::default()
        }),
        None,
    )?;
