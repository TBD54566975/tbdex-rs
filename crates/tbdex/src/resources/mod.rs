...
impl ResourceKind {
    pub fn typesafe_id(&self) -> Result<String> {
        ...
        let mut last_timestamp = crate::resources::LAST_TIMESTAMP.lock().unwrap();

        let counter_value = if *last_timestamp == timestamp {
            crate::resources::COUNTER.fetch_add(1, Ordering::SeqCst)
        } else {
            crate::resources::COUNTER.store(0, Ordering::SeqCst);
